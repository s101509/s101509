...
startButton.forEach((element) => {
	element.addEventListener('click', btnStart)
})
		...
		for (let j = 0; j < rows; j++) {
			if (currentBoard[i][j].value == 0) {
				...
				drawRect(
					...
					unitLength,
					unitLength,
					currentBoard[i][j].color[0],
					currentBoard[i][j].color[1],
					currentBoard[i][j].color[2],
					true
				)
			}
		}
		...
		for (let x = 0; x < columns; x++) {
			for (let y = 0; y < rows; y++) {
				if (currentBoard[x][y].movable) {
					...
					nextBoard[pivotPoint[0] - relativeY][
						pivotPoint[1] + relativeX
					] = { ...currentBoard[x][y] }
				}
			}
		}
...
function generate() {
	...
	if (gameOver) {
		alert('Game over!')
		gameState = 'Stop'
	} else currentBoard = copyBoard(nextBoard)
}
